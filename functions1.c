				...
				int textwidth, int valuePrecision, int typeSize)
{
	...
	return (write_unsgnd(0, i, tempBuffer, activieFlags,
...
int print_hexadecimal(va_list argslist, char tempBuffer[], int activieFlags,
				int textwidth, int valuePrecision, int typeSize)
{
	return (print_hexa(argslist, "0123456789abcdef", tempBuffer, activieFlags,
				'x', textwidth, valuePrecision, typeSize));
}
...
int print_hexa_upper(va_list argslist, char tempBuffer[], int activieFlags,
				int textwidth, int valuePrecision, int typeSize)
{
	return (print_hexa(argslist, "0123456789ABCDEF", tempBuffer,
				activieFlags, 'X', textwidth, valuePrecision, typeSize));
}

/************** PRINT HEXX NUM IN LOWER OR UPPER **************/

int print_hexa(va_list argslist, char map_to[], char tempBuffer[], int
activieFlags, char flag_ch, int textwidth, int valuePrecision, int typeSize)
{
	int i = BUFF_SIZE - 2;
	unsigned long int num = va_arg(argslist, unsigned long int);
	unsigned long int init_num = num;
